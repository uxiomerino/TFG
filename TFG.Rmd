...
```{r}
df_combis_Obradoiro <- df_combinaciones[df_combinaciones$Equipo == "Monbus Obradoiro" & df_combinaciones$Ano == "2023/24", ]
dim(df_combis_Obradoiro)
head(df_combis_Obradoiro)
```

```{r}
df_combis_Obradoiro[which.max(df_combis_Obradoiro$Diferencia), ]
df_combis_Obradoiro[which.max(df_combis_Obradoiro$Minutos), ]
df_combis_Obradoiro[which.min(df_combis_Obradoiro$Diferencia), ]
df_combis_Obradoiro[which.min(df_combis_Obradoiro$Minutos), ]
```

```{r}
df_combis_Obradoiro5 <- df_combis_Obradoiro[df_combis_Obradoiro$Minutos > 5, ]
dim(df_combis_Obradoiro5)
head(df_combis_Obradoiro5)
```

```{r}
df_quintetosObra <- pasoClusters(df_combis_Obradoiro5)
head(df_quintetosObra)
```

```{r}
axuste_DiferenciaObra <- lm(Media~C1+C2+C3+C4+C5+C6, data = df_quintetosObra)
summary(axuste_DiferenciaObra)
```

```{r}
plot(axuste_DiferenciaObra)
```
