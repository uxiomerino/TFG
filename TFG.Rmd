...
busqueda_xogadores_cercanos(df_dist, "22 EDY TAVARES", distancia = 5, año = "2023/24")[1]
busqueda_xogadores_cercanos(df_dist, "13 SERGIO RODRIGUEZ", distancia = 5)[1]
```
...
clusters_jerarquico <- cutree(clustering_jerarquico, k = 6)
...
df_combinaciones <- read.csv("boxscore_combis_obra.csv")
df_combinaciones$Ano <- rep("2022/23", length(df_combinaciones$Equipos))

# Sobrescribir el dataframe original con solo las columnas que queremos conservar
df_combinaciones <- df_combinaciones[, c("Equipos", "Xogadores", "Minutos", "Partidos", "Dif")]
...
```{r}
library(tidyr)
df_combinaciones <- separate(df_combinaciones, Xogadores, into = c("Xogador 1", "Xogador 2", "Xogador 3", "Xogador 4", "Xogador 5"), sep = "(?<=\\D) (?=\\d)")
head(df_combinaciones)
```

```{r}
df_combinaciones[which.max(df_combinaciones$Minutos), ]
df_combinaciones[which.max(df_combinaciones$Dif), ]
df_combinaciones[which.min(df_combinaciones$Minutos), ]
```


```{r}
xogadores_validos <- unique(df_union[df_union$Equipo == "Monbus Obradoiro" & df_union$Año == "2022/23", "Xogador"])
cluster_1 <- c()
cluster_2 <- c()
cluster_3 <- c()
cluster_4 <- c()
...
df_quintetos$Dif <- df_combinaciones$Dif
df_quintetos$C1 <- as.factor(df_quintetos$C1)
df_quintetos$C2 <- as.factor(df_quintetos$C2)
df_quintetos$C3 <- as.factor(df_quintetos$C3)
df_quintetos$C4 <- as.factor(df_quintetos$C4)
df_quintetos$C5 <- as.factor(df_quintetos$C5)
df_quintetos$C6 <- as.factor(df_quintetos$C6)
head(df_quintetos)
...
```{r}
axuste1 <- lm(Dif~C1*C2*C3*C4*C5*C6, data = df_quintetos)
summary(axuste1)
```
