...
```{r}
victoriasObra <- lm(formula = Resultado ~ PPT + PPT2 + TAsist + T2Asist + 
    TL_Min + TL_F + RebTot + `%TL` + Asist + TO + Rec + TRea + 
    FRea + mediaDifRival, data = partidosObra)
...
```{r}
victoriasObra <- lm(formula = Resultado ~ PPT + PPT2 + TAsist + 
    TL_Min + RebTot + `%TL` + Asist + TO + Rec + TRea + 
    FRea + mediaDifRival, data = partidosObra)
...
```{r}
victoriasObra <- lm(formula = Resultado ~ PPT + TAsist + 
    TL_Min + RebTot + `%TL` + TO + Rec + TRea + 
    FRea + mediaDifRival, data = partidosObra)
...
```{r}
victoriasObra <- lm(formula = Resultado ~ PPT + TAsist + 
    TL_Min + RebTot + TO + Rec + FRea + mediaDifRival, data = partidosObra)

summary(victoriasObra)
```

```{r}
victoriasObra <- lm(formula = Resultado ~ PPT + TL_Min + RebTot + TO +
                      Rec + FRea + mediaDifRival, data = partidosObra)

summary(victoriasObra)
```

```{r}
victoriasObra <- lm(formula = Resultado ~ PPT + RebTot + TO + Rec + FRea + mediaDifRival, data = partidosObra)
...
varsObra3 <- rfe(x = quintetos_obra_limpos[, -22], 
                 y = quintetos_obra_limpos$DiferenciaPorMin, 
                 sizes = c(1:(ncol(quintetos_obra_limpos)-1)),
                 rfeControl = control_rfe)

predictors(varsObra3)
```
...
set.seed(123)
randomForestObra2 <- train(DiferenciaPorMin ~ ., data = quintetos_obra_limpos[, cols_RF_Obra], method = "rf", trControl = ctrl, tuneGrid = tuneGrid)
...
SSE <- sum((predsObraAxustadas - DifPMin_orixinal)^2)
SST <- sum((DifPMin_orixinal - mean(DifPMin_orixinal))^2)
(R2 <- 1 - SSE/SST)

(MAE <- mean(abs(predsObraAxustadas - DifPMin_orixinal)))

(MAE / sd(DifPMin_orixinal))
```

```{r}
datos_preds <- data.frame(obs = DifPMin_orixinal, pred = predsObraAxustadas)
sum((datos_preds$obs < 0) != (datos_preds$pred < 0))
...
quinteto_max_pred <- quintetos_obra2_limpos[which.max(predsObra2), c("Cluster1", "Cluster2", "Cluster3", "Cluster4", "Cluster5", "Cluster6")]
quinteto_max_pred$Predicion <- axustarDifPorMin(max(predsObra2))
...
non_expl_max <- non_explorados[which.max(predsObra3), c("Cluster1", "Cluster2", "Cluster3", "Cluster4", "Cluster5", "Cluster6")]
non_expl_max$Predicion <- axustarDifPorMin(max(predsObra3))
