...
quintetos_global <- quintetos_global[quintetos_global$Minutos > 5, ]
quintetos_global
```
...
quintetos_global <- separate(quintetos_global, Xogadores, into = c("Xogador 1", "Xogador 2", "Xogador 3", "Xogador 4", "Xogador 5"), sep = "(?<=\\D) (?=\\d)")
...
for (i in 1:nrow(quintetos_global)) {
  quinteto <- quintetos_global[i, c("Xogador 1", "Xogador 2", "Xogador 3", "Xogador 4", "Xogador 5")]
...
quintetos_global <- quintetos_global[-indices, ]

quintetos_global$Cluster1 <- cluster_1
quintetos_global$Cluster2 <- cluster_2
quintetos_global$Cluster3 <- cluster_3
quintetos_global$Cluster4 <- cluster_4
quintetos_global$Cluster5 <- cluster_5

quintetos_global$Cluster1 <- as.factor(quintetos_global$Cluster1)
quintetos_global$Cluster2 <- as.factor(quintetos_global$Cluster2)
quintetos_global$Cluster3 <- as.factor(quintetos_global$Cluster3)
quintetos_global$Cluster4 <- as.factor(quintetos_global$Cluster4)
quintetos_global$Cluster5 <- as.factor(quintetos_global$Cluster5)

quintetos_global <- quintetos_global[, -c(1, 2, 3, 4, 5)]

df_quintetos <- data.frame(matrix(ncol = 6, nrow = nrow(quintetos_global)))
colnames(df_quintetos) <- c("C1", "C2", "C3", "C4", "C5", "C6")

df_parcial <- quintetos_global
...
quintetos_global$Cluster1 <- df_quintetos$C1
quintetos_global$Cluster2 <- df_quintetos$C2
quintetos_global$Cluster3 <- df_quintetos$C3
quintetos_global$Cluster4 <- df_quintetos$C4
quintetos_global$Cluster5 <- df_quintetos$C5
quintetos_global$Cluster6 <- df_quintetos$C6

medias <- quintetos_global %>%
group_by(Equipo) %>%
 summarise(media = mean(Diferencia))
  
head(medias)
  
quintetos_global <- left_join(quintetos_global, medias, by = "Equipo")
  
quintetos_global <- quintetos_global %>%
  mutate(Diferencia = Diferencia - media) 

quintetos_global <- quintetos_global[, -c(1, 2)]#, 25)]
...
## Proba regresión 
```{r}
axuste_lm <- lm(Diferencia~., data = quintetos_global)
summary(axuste_lm)
```
...
```{r}
MOD_NULL <- lm(Diferencia ~ 1, data = quintetos_global)
MOD_FULL <- axuste_lm
...
umbral <- 1/(1-summary(stepMod)$r.squared)
print(FIV[FIV>umbral])
cor(quintetos_global[, c('PPPos', 'PPT', 'PPT2', 'TAsist', 'T2Asist', 'T3Asist', 'TO')])
```

```{r}
axuste_lm <- lm(formula = Diferencia ~ PPPos + PPT + TAsist 
                + TL_F + RebDef + RebOf + TO + Cluster4,
    data = quintetos_global)
summary(axuste_lm)
...
cor(quintetos_global[, c('PPPos', 'PPT' ,'TO')])
```

```{r}
axuste_lm <- lm(formula = Diferencia ~ PPT + TAsist + TL_F + RebDef 
                + RebOf + TO + Cluster4,
    data = quintetos_global)
summary(axuste_lm)
...
```{r}
axuste_lm <- lm(formula = Diferencia ~ PPT + TL_F + RebDef 
                + RebOf + TO + Cluster4,
    data = quintetos_global)
summary(axuste_lm)
```
...
tamano_adestram <- round(0.80 * nrow(quintetos_global))

indices_adestram <- sample(1:nrow(quintetos_global), tamano_adestram, replace = FALSE)

train_obra <- quintetos_global[indices_adestram, ]
test_obra <- quintetos_global[-indices_adestram, ]
...
( R2(preds, test_obra$Diferencia) )

sd_true <- sd(test_obra$Diferencia)

mae_relative_sd <- mae / sd_true

print(paste("MAE relativo respecto a la desviación estándar:", mae_relative_sd))
```
