...
quintetos_obra_limpos_norm <- as.data.frame(scale(quintetos_obra_limpos[, -17]))
quintetos_obra_limpos_norm$Diferencia <- quintetos_obra_limpos$Diferencia
...
set.seed(32725752)
# Performing RFE
varsObra <- rfe(x = quintetos_obra_limpos_norm[, -23], 
...
```{r}
range(quintetos_obra_limpos_norm$Diferencia)
randomForestObra$results$MAE[randomForestObra$bestTune$mtry] / sd(quintetos_obra_limpos_norm$Diferencia)
```

```{r}
preds <- predict(randomForestObra)
datos_preds <- data.frame(obs = quintetos_obra_limpos_norm$Diferencia, pred = preds)
...
columnas <- c('Cluster1', 'Cluster2', 'Cluster3', 'Cluster4', 'Cluster5', 'Cluster6')
non_explorados <- anti_join(quintetos_obra2_limpos, quintetos_obra_limpos, by = columnas)

mediasObra <- colMeans(quintetos_obra_limpos)
desviacionsObra <- apply(quintetos_obra_limpos, 2, sd)
non_explorados_norm <- as.data.frame(scale(non_explorados, center = mediasObra, scale = desviacionsObra))

predsObra2 <- predict(randomForestObra, non_explorados_norm)

quinteto_max_pred <- quintetos_obra2_limpos[which.max(predsObra2), c("Cluster1", "Cluster2", "Cluster3", "Cluster4", "Cluster5", "Cluster6")]
quinteto_max_pred$Predicion <- max(predsObra2)
quinteto_max_pred$Xogadores <- quintetos_obra2[which.max(predsObra2), "Xogadores"]

quinteto_max_pred
```

```{r}
hist(quintetos_obra_limpos_norm$Diferencia, breaks = 10)
abline(v = max(predsObra2), col = "blue", lwd = 2)
```
