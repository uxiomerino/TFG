...
MSSR <- summary(axuste_interaccions)$sigma^2
res <- residuals(axuste_interaccions)
res.est <- res/sqrt(MSSR)
res.stud <- studres(axuste_interaccions) 


# standarizados vs studentizados
plot(axuste_interaccions$fitted.values, res.est, 
     type="p",col=4,pch=19, xlab="Axustados",ylab="Residuos", 
     ylim=range(cbind(res.est,res.stud)))
points(axuste_interaccions$fitted.values,res.stud, col=2,pch=17)
legend("topleft",c("Estandarizados","Studentizados"),
       pch=19,col=c(4,2),lty=c(-1,-1))


scatter.smooth(axuste_interaccions$fit, res.est, main="Residuos ~ Axustados", 
               xlab="Axustados",ylab="Residuos", pch = 21, 
...
hist(res.est, breaks="FD",freq=FALSE, main = "", xlab="Residuos", cex.lab=1.4, 
...
moments::agostino.test(res.est) # Prueba de D'Agostino

plot(axuste_interaccions, which=3)
# H0: sigma^2=cte  vs H1: sigma^2!=cte
# Test de Breusch-Pagan
ncvTest(axuste_interaccions) 

Box.test(res.est, lag = 5, type = "Ljung-Box")	   	 # Prueba de Ljung-Box 
```

 
## Modelo non paramétrico:

```{r}
loess(Dif ~ ., data = df_quintetos)
```
...
axuste_victorias <- glm(Resultado ~ PPPos + PPT + PPT2 + PPT3 + TAsist + T2Asist + T3Asist + TL_Min + TL_F + RebTot + RebOf + RebDef + `%TL` + Asist + TO + Rec + TRea + FRea, data = df_equipos)
summary(axuste_victorias)
shapiro.test(resid(axuste_victorias))

qqnorm(resid(axuste_victorias))
qqline(resid(axuste_victorias))

AIC(axuste_victorias)
BIC(axuste_victorias)
```
```{r}
