...
for (i in 1:4){
  for (j in 2:5){
    if (i != j)    plot(fviz_pca_ind(pca, axes = c(i, j), col.ind = as.factor(clusters$cluster)))
  }
}
```
...
```{r}
clustering_jerarquico <- hclust(dist(df_clusters, method = "mahalanobis"), method = "ward.D2")
clusters_jerarquico <- cutree(clustering_jerarquico, k = 7)
