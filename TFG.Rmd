...
```{r}
df_combinaciones <- df_combinaciones[df_combinaciones$Minutos > 2.30, ]
head(df_combinaciones)
```
...
modelo_rf <- randomForest(Dif ~ C1*C2*C3*C4*C5*C6, data = df_quintetos)
...
modelo_arbol <- rpart(Dif ~ C1+C2+C3+C4+C5+C6, data = df_quintetos, method = "anova")
```
