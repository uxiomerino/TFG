...
clusters_fuzzy <- Fclust(df_clusters, k = 6)

summary(clusters_fuzzy)

Hraw(df_clusters, clusters_fuzzy$H)
```
...
## Probas de modelos:
```{r}
df_quintetos$Dif_Transf <- df_quintetos$Dif + abs(min(df_quintetos$Dif)) + 1

axuste_glm <- glm(Dif_Transf~C1*C2*C3*C4*C5*C6, data = df_quintetos, family = Gamma)
summary(axuste_glm)
```

```{r}
plot(residuals(axuste_glm))
plot(axuste_glm)
```
