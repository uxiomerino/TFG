...
```{r}
axuste_lm <- lm(Diferencia~., data = quintetos_obra_limpos_norm)
summary(axuste_lm)
```
...
```{r}
MOD_NULL <- lm(Diferencia ~ 1, data = quintetos_obra_limpos_norm)
MOD_FULL <- axuste_lm
...
axuste_lm <-lm(formula = Diferencia ~ PPMin + PPT + T3Asist + RebOf + 
    Cluster5, data = quintetos_obra_limpos_norm)
...
## Proba RandomForest Obra
```{r}
library(caret)
library(randomForest)

# tamano_adestram <- round(0.80 * nrow(quintetos_obra_limpos))
# indices_adestram <- sample(1:nrow(quintetos_obra_limpos), tamano_adestram, replace = FALSE)

# train_obra <- quintetos_obra_limpos[indices_adestram, ]
# test_obra <- quintetos_obra_limpos[-indices_adestram, ]

# train_mean <- colMeans(train_obra)
# train_sd <- apply(train_obra, 2, sd)

# train_obra_norm <- as.data.frame(scale(train_obra, center = train_mean, scale = train_sd))
# test_obra_norm <- as.data.frame(scale(test_obra, center = train_mean, scale = train_sd))
...
set.seed(9136)
# Performing RFE
varsObra <- rfe(x = quintetos_obra_limpos_norm[, -16], 
                 y = quintetos_obra_limpos_norm$Diferencia, 
                 sizes = c(1:(length(colnames(quintetos_obra_limpos_norm))-1)),
...
randomForestObra <- train(Diferencia ~ ., data = quintetos_obra_limpos_norm[, cols_RF_Obra], method = "rf", trControl = ctrl, tuneGrid = tuneGrid)
...
```{r}
preds <- predict(randomForestObra, newdata = test_obra_norm)

datos_preds <- data.frame(obs = test_obra_norm$Diferencia, pred = preds)


( mae <- mean(abs(preds-test_obra_norm$Diferencia)) )
( mse <- mean((preds-test_obra_norm$Diferencia)^2) )
( R2(preds, test_obra_norm$Diferencia) )
