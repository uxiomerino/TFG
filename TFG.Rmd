...
str(df)
df$Competicion <- rep("LigaACB", dim(df)[1])
...
df3$Competicion <- rep("LigaACB", dim(df3)[1])
...
df_dist <- data.frame(coord_indiv[, 1:5])
df_dist$Xogador <- df_pca$Xogador
df_dist$Equipo <- df_pca$Equipo
df_dist$Competicion <- df_pca$Competicion
...
distancia <- function(df, x1, ano1, x2, ano2, method = "Euclidean") {
  xogador1 <- df[df$Xogador == x1 & df$Año == ano1, c('Dim.1', 'Dim.2', 'Dim.3', 'Dim.4', 'Dim.5')]
  xogador2 <- df[df$Xogador == x2 & df$Año == ano2, c('Dim.1', 'Dim.2', 'Dim.3', 'Dim.4', 'Dim.5')]
...
```{r}
clusters <- hkmeans(df_clusters, k = 6)
...
```{r}
ordered_clustered_data[ordered_clustered_data$CLUSTER==3,]
```
