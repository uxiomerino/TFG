...
```{r}
load("~/GCED/TFG/Datos/quintetos_obradoiro.RData")
quintetos_obradoiro <- quintetos_obradoiro[quintetos_obradoiro$Minutos > 5, ]
quintetos_obradoiro
```
...
quintetos_obradoiro <- separate(quintetos_obradoiro, Xogadores, into = c("Xogador 1", "Xogador 2", "Xogador 3", "Xogador 4", "Xogador 5"), sep = "(?<=\\D) (?=\\d)")
...
for (i in 1:nrow(quintetos_obradoiro)) {
  quinteto <- quintetos_obradoiro[i, c("Xogador 1", "Xogador 2", "Xogador 3", "Xogador 4", "Xogador 5")]
  xogadores_quinteto <- unlist(quinteto)
...
quintetos_obradoiro <- quintetos_obradoiro[-indices, ]

quintetos_obradoiro$Cluster1 <- cluster_1
quintetos_obradoiro$Cluster2 <- cluster_2
quintetos_obradoiro$Cluster3 <- cluster_3
quintetos_obradoiro$Cluster4 <- cluster_4
quintetos_obradoiro$Cluster5 <- cluster_5

quintetos_obradoiro$Cluster1 <- as.factor(quintetos_obradoiro$Cluster1)
quintetos_obradoiro$Cluster2 <- as.factor(quintetos_obradoiro$Cluster2)
quintetos_obradoiro$Cluster3 <- as.factor(quintetos_obradoiro$Cluster3)
quintetos_obradoiro$Cluster4 <- as.factor(quintetos_obradoiro$Cluster4)
quintetos_obradoiro$Cluster5 <- as.factor(quintetos_obradoiro$Cluster5)

quintetos_obradoiro <- quintetos_obradoiro[, -c(1, 2, 3, 4, 5, 6)]

df_quintetos <- data.frame(matrix(ncol = 6, nrow = nrow(quintetos_obradoiro)))
colnames(df_quintetos) <- c("C1", "C2", "C3", "C4", "C5", "C6")

df_parcial <- quintetos_obradoiro
for (i in (1:nrow(df_parcial))){
  clusters_quinteto <- numeric(6)
  for (j in (1:ncol(df_parcial))){
    if (df_parcial[i, j] == 1) clusters_quinteto[1] = clusters_quinteto[1] + 1
...
quintetos_obradoiro$Cluster1 <- df_quintetos$C1
quintetos_obradoiro$Cluster2 <- df_quintetos$C2
quintetos_obradoiro$Cluster3 <- df_quintetos$C3
quintetos_obradoiro$Cluster4 <- df_quintetos$C4
quintetos_obradoiro$Cluster5 <- df_quintetos$C5
quintetos_obradoiro$Cluster6 <- df_quintetos$C6
...
## Proba regresión 
```{r}
axuste_lm <- lm(Diferencia~., data = quintetos_obradoiro)
summary(axuste_lm)
```
...
```{r}
MOD_NULL <- lm(Diferencia ~ 1, data = quintetos_obradoiro)
MOD_FULL <- axuste_lm
...
tamano_adestram <- round(0.90 * nrow(quintetos_obradoiro))
...
tuneGrid <- expand.grid(mtry = c(1:(ncol(train_obra) - 1)))
folds <- c(3, 5, 10)
erros <- numeric(3)
modelos <- list()

for (i in 1:length(folds)){
  ctrl <- trainControl(method = "cv", number = folds[i])
  randomForestObra <- train(Diferencia ~ ., data = train_obra, method = "rf", trControl = ctrl, tuneGrid = tuneGrid)
  modelos <- c(modelos, randomForestObra)
  erros[i] <- randomForestObra$results[which.min(randomForestObra$results[, 2]), 2]
}
...
```{r}
preds <- predict(randomForestObra, newdata = test_obra)
( mae <- mean(abs(preds-test_obra$Diferencia)) )
( mse <- mean((preds-test_obra$Diferencia)^2) )
```

## Proba con SVM:
```{r}
library(e1071)
tuneGrid <- list(kernel = c("linear", "polynomial", "sigmoid", "radial"),
                        degree = c(2, 3, 4),
...
for (k in tuneGrid$kernel){
  for (d in tuneGrid$degree){
    for (g in tuneGrid$gamma){
      for (c in tuneGrid$coef0){
        for (cost in tuneGrid$cost){
          for (s in tuneGrid$sigma){
            for (cross in tuneGrid$cross){
              for (sc in tuneGrid$sc){
                svm_model <- svm(Diferencia ~ ., data = train_obra, kernel = k,
                                 degree = d, gamma = g, coef0 = c, cost = cost, 
                                 sigma = s, cross = cross, scale = sc)
                preds <- predict(svm_model, test_obra)
                mse <- mean((preds-test_obra$Diferencia)^2)
                if (mse < best_mse) {
                  best_mse <- mse
                  best_svm <- svm_model
...
preds <- predict(best_svm, test_obra)
print(mean(abs(preds-test_obra$Diferencia)))
```




## Combinacións Obradoiro para predición e selección do valor óptimo:
```{r}
