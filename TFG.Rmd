...
```{r}
axuste_victorias2 <- lm(Resultado~.-Xornada-PPT3, data = df_equipos)
anova(axuste_victorias, axuste_victorias2, test="Chisq")
drop1(axuste_victorias2, test = "Chisq")
```
```{r}
axuste_victorias3 <- lm(Resultado~.-Xornada-Asist-PPT3, data = df_equipos)
anova(axuste_victorias2, axuste_victorias3, test="Chisq")
drop1(axuste_victorias3, test = "Chisq")
```
```{r}
axuste_victorias4 <- lm(Resultado~.-Xornada-Asist-PPT3-T2Asist, data = df_equipos)
anova(axuste_victorias3, axuste_victorias4, test="Chisq")
drop1(axuste_victorias4, test = "Chisq")
```
```{r}
axuste_victorias5 <- lm(Resultado~.-Xornada-Asist-PPT3-T2Asist-`%TL`, data = df_equipos)
anova(axuste_victorias4, axuste_victorias5, test = "Chisq")
drop1(axuste_victorias5, test = "Chisq")
```

```{r}
axuste_victorias6 <- lm(Resultado~.-Xornada-Asist-PPT3-T2Asist-`%TL`-PPT2, data = df_equipos)
anova(axuste_victorias5, axuste_victorias6, test = "Chisq")
drop1(axuste_victorias6, test = "Chisq")
```

```{r}
axuste_victorias7 <- lm(Resultado~.-Xornada-Asist-PPT3-T2Asist-`%TL`-PPT2-PPPos, data = df_equipos)
anova(axuste_victorias6, axuste_victorias7, test = "Chisq")
drop1(axuste_victorias7, test = "Chisq")
```

```{r}
axuste_victorias8 <- lm(Resultado~.-Xornada-Asist-PPT3-T2Asist-`%TL`-PPT2-PPPos-TL_Min, data = df_equipos)
anova(axuste_victorias7, axuste_victorias8, test = "Chisq")
drop1(axuste_victorias8, test = "Chisq")
```

```{r}
axuste_victorias9 <- lm(Resultado~.-Xornada-Asist-PPT3-T2Asist-`%TL`-PPT2-PPPos-TL_Min-FRea, data = df_equipos)
anova(axuste_victorias8, axuste_victorias9, test = "Chisq")
drop1(axuste_victorias9, test = "Chisq")
```

```{r}
axuste_victorias10 <- lm(Resultado~.-Xornada-Asist-PPT3-T2Asist-`%TL`-PPT2-PPPos-TL_Min-FRea-T3Asist, data = df_equipos)
anova(axuste_victorias9, axuste_victorias10, test = "Chisq")
drop1(axuste_victorias10, test = "Chisq")
```

```{r}
axuste_victorias11 <- lm(Resultado~.-Xornada-Asist-PPT3-T2Asist-`%TL`-PPT2-PPPos-TL_Min-FRea-T3Asist-TL_F, data = df_equipos)
anova(axuste_victorias10, axuste_victorias11, test = "Chisq")
drop1(axuste_victorias11, test = "Chisq")
```

```{r}
axuste_victorias12 <- lm(Resultado~.-Xornada-Asist-PPT3-T2Asist-`%TL`-PPT2-PPPos-TL_Min-FRea-T3Asist-TL_F-TRea, data = df_equipos)
...
```{r}
summary(axuste_victorias11)

vif(axuste_victorias11)
```

```{r}
axuste_victorias13 <- lm(Resultado~.-Xornada-Asist-PPT3-T2Asist-`%TL`-PPT2-PPPos-TL_Min-FRea-T3Asist-TL_F-RebTot, data = df_equipos)
...
```{r}
load("~/GCED/TFG/Datos/df_RebDef.RData")
df_RebDef$Ano <- rep("2023/24", length(df_RebDef$Equipo))
...
```{r}
df_combinaciones <- df_combinaciones[df_combinaciones$Minutos > 2.5, ]
head(df_combinaciones)
```
...
```{r}
axuste_RebOf <- lm(Media~C1*C2*C3*C4*C5*C6, data = df_quintetos)
summary(axuste_RebOf)
```
