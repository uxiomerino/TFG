...
## Introducir axuste Bilbao Basket aquí para a mostra
...
```{r}
axuste_Diferencia <- lm(Media~C1+C2+C3+C4+C5+C6, data = df_quintetos)
summary(axuste_Diferencia)
```

```{r}
plot(axuste_Diferencia)
```

```{r}
shapiro.test(axuste_Diferencia$residuals)
```
...
combinaciones_obradoiro <- combinaciones_obradoiro[combinaciones_obradoiro$Minutos > 10, ]
head(combinaciones_obradoiro)
```
...
```{r}
agrupaciones_obradoiro <- quintetos_obradoiro %>%
  group_by(C1, C2, C3, C4, C5, C6) %>%
  summarize(Suma_Media = sum(Media), Suma_Minutos = sum(Minutos)) %>%
  arrange(desc(Suma_Media))

print(agrupaciones_obradoiro)
```
...
## Axuste cos xogadores nas vars significativas
```{r}
load("~/GCED/TFG/Datos/RebTot_Obradoiro.RData")
head(RebTot_Obradoiro)
```

```{r}
RebTot_Obradoiro <- separate(RebTot_Obradoiro, Xogadores, into = c("Xogador 1", "Xogador 2", "Xogador 3", "Xogador 4", "Xogador 5"), sep = "(?<=\\D) (?=\\d)")
n_miss(RebTot_Obradoiro)
head(RebTot_Obradoiro)
tail(RebTot_Obradoiro)
```



```{r}
RebTot_Obradoiro <- RebTot_Obradoiro[RebTot_Obradoiro$Minutos > 10, ]
...
for (i in 1:nrow(RebTot_Obradoiro)) {
  quinteto <- RebTot_Obradoiro[i, c("Xogador 1", "Xogador 2", "Xogador 3", "Xogador 4", "Xogador 5")]
  xogadores_quinteto <- unlist(quinteto)
...
RebTot_Obradoiro <- RebTot_Obradoiro[-indices, ]

RebTot_Obradoiro$Cluster1 <- cluster_1
RebTot_Obradoiro$Cluster2 <- cluster_2
RebTot_Obradoiro$Cluster3 <- cluster_3
RebTot_Obradoiro$Cluster4 <- cluster_4
RebTot_Obradoiro$Cluster5 <- cluster_5

quintetosRebTot_obra <- pasoClusters(RebTot_Obradoiro)
quintetosRebTot_obra$Metrica <- RebTot_Obradoiro$`Valor Equipo`
quintetosRebTot_obra$Minutos <- RebTot_Obradoiro$Minutos

head(quintetosRebTot_obra)
```

```{r}
axuste_Rebotes <- lm(Metrica~C1+C2+C3+C4+C5+C6, data = quintetosRebTot_obra)
summary(axuste_Rebotes) # Multicoliñalidade perfecta, que facer?
```

```{r}
frecuencias <- quintetosRebTot_obra %>%
  group_by_at(vars(-Metrica, -Minutos)) %>%
  summarise(count = n(), .groups = 'drop') %>%
  arrange(desc(count))
