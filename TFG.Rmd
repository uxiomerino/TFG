...
quintetos_obra <- quintetos_global[quintetos_global$Equipo == "Monbus Obradoiro" &
                                     quintetos_global$Minutos > 10, ]
quintetos_obra

quintetos_obra2 <- quintetos_global[quintetos_global$Equipo == "Monbus Obradoiro" &
                 quintetos_global$Minutos >= 5 & quintetos_global$Minutos <= 10, ]
...
summary(quintetos_obra_limpos_norm)
```
...
```{r}
mean(randomForestObra$results$MAE) / sd(quintetos_obra_limpos$Diferencia)
(datos_preds$obs < 0) == (datos_preds$pred < 0)
```

```{r}
quintetos_obra2_limpos <- preprocesado_quintetos(quintetos_obra2)

mediasObra <- colMeans(quintetos_obra_limpos)
desviacionsObra <- apply(quintetos_obra_limpos, 2, sd)

quintetos_obra2_limpos_norm <- as.data.frame(scale(quintetos_obra2_limpos, center = mediasObra, scale = desviacionsObra))
quintetos_obra2_limpos_norm
```

### Predición sobre quintetos non explorados:
```{r}
predsObra2 <- predict(randomForestObra, quintetos_obra2_limpos_norm)
