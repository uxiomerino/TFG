...
umbral <- 1/(1-summary(stepMod)$r.squared)
print(FIV[FIV>umbral])
cor(quintetos_global[, c('PPPos','TAsist', 'T2Asist', 'T3Asist', 'Asist')])
```

```{r}
axuste_lm <- lm(formula = Diferencia ~ PPPos + TAsist + RebDef + Asist + 
                  TO + Cluster3 + Cluster4, data = quintetos_global)
summary(axuste_lm)
...
cor(quintetos_global[, c('PPPos', 'TAsist' ,'Asist')])
```

```{r}
axuste_lm <- lm(formula = Diferencia ~ PPPos + TAsist + RebDef +  TO 
                + Cluster3 + Cluster4, data = quintetos_global)
summary(axuste_lm)
...
```{r}
axuste_lm <- lm(formula = Diferencia ~ PPPos + RebDef + TO , data = quintetos_global)
